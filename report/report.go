...
import (
	...
	"git.jwn.app/TM00458/Operations/scripts/ns-report-gen/servicenow"
)
...
func CreateCsvFile(r []ReportData, clusterList []string) {
	...
	for _, entry := range r {
		...
		if Contains(used, entry.appID) {
			...
			for _, slice := range legNS {
				if slice[1] == entry.appID {
					s[1] = slice[0]
					break
				}
			}
