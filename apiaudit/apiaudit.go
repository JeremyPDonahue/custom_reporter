...
func FilterIngressForAPI(resourceJSON []IngressJSON, apiVersion string) []APIAudit {
	...
	for _, resource := range resourceJSON {
		...
		// fmt.Printf("Filtering Ingress JSON for %s \n", cluster)
...
func GetGitLabJSON(projectID string) (string, string) {
	...
	output, err := cmd.Output()
	if err != nil {
		fmt.Printf("Curl command failed for projectID %s \n", projectID)
	}
...
func GetArchivedStatus(jsonStr, projectID string) (bool, error) {
	...
	if err := json.Unmarshal([]byte(jsonStr), &data); err != nil {
		fmt.Println("Could not unmarshal data while gathering archive status for proectID %s - Please confirm status manually.\n", projectID)
